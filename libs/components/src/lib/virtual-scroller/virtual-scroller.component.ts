...
export interface RwVirtualScrollerDefaultOptions {
...
export function VIRTUAL_SCROLLER_DEFAULT_OPTIONS_FACTORY(): RwVirtualScrollerDefaultOptions {
  return {
  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  public scrollToIndex(
    ...
    const retryIfNeeded = () => {
      ...
      const desiredStartIndex = Math.min(
        Math.max(index, 0),
  ...
  protected _invisiblePaddingProperty: 'scaleX' | 'scaleY';
  protected _offsetType: 'offsetLeft' | 'offsetTop';
  protected _scrollType: 'scrollLeft' | 'scrollTop';
  protected _pageOffsetType: 'pageXOffset' | 'pageYOffset';
  protected _childScrollDim: 'childWidth' | 'childHeight';
  protected _translateDir: 'translateX' | 'translateY';
  protected _marginDir: 'margin-left' | 'margin-top';
  ...
  protected debounce(func: Function, wait: number): Function {
    ...
    const result = function () {
      throttled['cancel']();
      throttled.apply(result, arguments);
    };
  ...
  protected currentTween: tween.Tween<unknown>;
  ...
  protected refresh_internal(
    ...
    this.zone.runOutsideAngular(() => {
      requestAnimationFrame(() => {
        ...
        const scrollLengthChanged =
          viewport.scrollLength !== this.previousViewPort.scrollLength;
        const paddingChanged =
          viewport.padding !== this.previousViewPort.padding;
      ...
      for (let i = 0; i < content.children.length; ++i) {
        ...
        if (arrayStartIndex % itemsPerWrapGroup === 0) {
          ...
          if (this.horizontal) {
            ...
            if (scrollOffset > 0) {
              const scrollOffsetToRemove = Math.min(
                scrollOffset,
  ...
  protected calculateViewport(): IViewport {
    ...
    const newPadding = this.calculatePadding(
